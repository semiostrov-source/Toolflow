...
- **Stack:** React 19 + TypeScript + Vite 8 (ESM, `"type": "module"`)
- **Test framework:** Vitest (latest) + `@testing-library/react` + `@testing-library/user-event` + `@testing-library/jest-dom`; environment: `jsdom`; globals enabled
- **File naming:** `*.test.ts` for pure logic/utilities, `*.test.tsx` for React components
- **File location:** Co-located with source files (e.g. `src/components/Button.test.tsx` next to `src/components/Button.tsx`)
- **Mock approach:** `vi.mock('module-path')` for module mocks, `vi.fn()` for function stubs, `vi.spyOn(obj, 'method')` for spies; call `vi.clearAllMocks()` in `beforeEach`
- **Test utilities:** Import `describe`, `it`, `expect`, `beforeEach`, `afterEach`, `vi` from `vitest`; use `render`, `screen`, `fireEvent`, `waitFor` from `@testing-library/react`; use `userEvent` from `@testing-library/user-event` for user interactions; `@testing-library/jest-dom` matchers (e.g. `toBeInTheDocument`) are available globally via `src/test/setup.ts`
- **Config:** `vitest.config.ts` at project root; setup file at `src/test/setup.ts`
- **Run commands:** `npm test` (watch), `npm run test:run` (single pass), `npm run test:ui` (UI)
- **Example reference:** `src/App.test.tsx`
