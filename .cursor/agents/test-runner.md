...
**Stack:** React 19 + TypeScript 5.9 + Vite 8 · Test runner: Vitest 4.x · Linter: ESLint 9 (flat config)

### Lint
```bash
npm run lint
```
> ESLint 9 flat config (`eslint.config.js`), targets `**/*.{ts,tsx}`. Plugins: `typescript-eslint`, `react-hooks`, `react-refresh`.

### Type Check
```bash
npx tsc -b
```
> Uses project references (`tsconfig.json` → `tsconfig.app.json`). `"noEmit": true` is set in `tsconfig.app.json` — type-checks without emitting JS.

### Test — All _(CI / agent use)_
```bash
npm run test:run
```
...
### Test — Watch Mode _(dev only, not for agents/CI)_
```bash
npm test
```
...
```
> ⚠️ Requires a coverage provider: `npm install -D @vitest/coverage-v8` (not yet installed).
